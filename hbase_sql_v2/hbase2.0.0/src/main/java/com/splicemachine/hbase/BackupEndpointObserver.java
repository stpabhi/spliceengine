 ...
 */
public class BackupEndpointObserver extends BackupBaseRegionObserver implements RegionCoprocessor {
    ...
    private ThreadLocal<Collection<HStoreFile>> storeFiles = new ThreadLocal<>();
    ...
    public void postCompact(ObserverContext<RegionCoprocessorEnvironment> e, Store store, StoreFile resultFile) throws IOException {
        try {
            ...
            if (LOG.isDebugEnabled()) {
                String filePath =  resultFile != null?((HStoreFile)resultFile).getFileInfo().getFileStatus().getPath().toString():null;
    ...
    public boolean postBulkLoadHFile(ObserverContext<RegionCoprocessorEnvironment> ctx, List<Pair<byte[], String>> familyPaths, boolean hasLoaded) throws IOException {
        ...
        try {
            ...
            try {
                ...
                byte[] family = familyPaths.get(0).getFirst();
                HStore store = region.getStore(family);
                Collection<HStoreFile> postBulkLoadStoreFiles = store.getStorefiles();
                for (HStoreFile storeFile : postBulkLoadStoreFiles) {
                    byte[] val = storeFile.getMetadataValue(HStoreFile.BULKLOAD_TASK_KEY);
