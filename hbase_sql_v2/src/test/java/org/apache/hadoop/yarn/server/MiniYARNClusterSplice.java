...
public class MiniYARNClusterSplice extends CompositeService {
     ...
     * @param numLogDirs the number of nm-log-dirs per nodemanager
     */
    public MiniYARNClusterSplice(String testName, int numNodeManagers,
                           int numLocalDirs, int numLogDirs) {
    ...
    private void setNonHARMConfiguration(Configuration conf) {
        String hostname = MiniYARNClusterSplice.getHostname();
    ...
    private class NodeManagerWrapper extends AbstractService {
        ...
        private String prepareDirs(String dirType, int numDirs) {
            ...
            for (int i = 0; i < numDirs; i++) {
                dirs[i]= new File(testWorkDir, MiniYARNClusterSplice.this.getName()
