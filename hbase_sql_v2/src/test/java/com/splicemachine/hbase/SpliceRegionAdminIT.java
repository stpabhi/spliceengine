...
public class SpliceRegionAdminIT {
    ...
    public void testTable() throws Exception {
        ...
        List<RegionInfo> partitions = admin.getRegions(tn);
    ...
    public void testDeleteAndMergeRegion() throws Exception {
        ...
        List<RegionInfo> partitions = admin.getRegions(tableName);
        for (RegionInfo partition : partitions) {
        ...
        while (partitions.size() != 1) {
            Thread.sleep(1000);
            partitions = admin.getRegions(tableName);
        }
        ...
        partitions = admin.getRegions(indexName);
        for (RegionInfo partition : partitions) {
            byte[] startKey = partition.getStartKey();
            if (startKey.length == 0) {
                String encodedRegionName = partition.getEncodedName();
