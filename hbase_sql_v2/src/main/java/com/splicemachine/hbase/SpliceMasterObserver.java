 ...
 */
@CoreCoprocessor
public class SpliceMasterObserver implements MasterObserver, MasterCoprocessor {
    ...
    public void start(CoprocessorEnvironment ctx) throws IOException {
        try {
            ...
            ZKWatcher zkw = ((HasMasterServices) ctx).getMasterServices().getZooKeeper();
        ...
        try {
            ...
            if (Bytes.equals(desc.getTableName().getName(), INIT_TABLE)) {
                switch(manager.getState()){
                    case NOT_STARTED:
                    	boot(((HasMasterServices)ctx.getEnvironment()).getMasterServices().getServerName());
                    case BOOTING_ENGINE:
