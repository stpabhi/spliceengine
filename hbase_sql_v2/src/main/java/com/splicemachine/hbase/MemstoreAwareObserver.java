...
public class MemstoreAwareObserver implements CompactionObserver, RegionObserver, RegionCoprocessor,
        SplitObserver,
        FlushObserver,
    ...
    public void preClose(ObserverContext<RegionCoprocessorEnvironment> c, boolean abortRequested) throws IOException {
        try {
            ...
            while (true) {
                MemstoreAware latest = memstoreAware.get();
                if (latest.currentScannerCount>0 && !((HasRegionServerServices)c.getEnvironment()).getRegionServerServices().isAborted()) {
        ...
        try {
            if (scan.getAttribute(MRConstants.SPLICE_SCAN_MEMSTORE_ONLY) != null &&
                ...
                if (Bytes.equals(startKey,c.getEnvironment().getRegionInfo().getStartKey()) &&
                    ...
                    Bytes.equals(serverName,Bytes.toBytes(((HasRegionServerServices)c.getEnvironment()).getRegionServerServices().getServerName().getHostAndPort()))
                    ) {
                    // Partition Hit
                    InternalScan iscan = new InternalScan(scan);
                    iscan.checkOnlyMemStore();
                    HRegion region = (HRegion) c.getEnvironment().getRegion();
                    return new MemStoreFlushAwareScanner(region, (HStore) store, ((HStore)store).getScanInfo(), iscan, targetCols, getReadpoint(region), memstoreAware, memstoreAware.get());
                } else { // Partition Miss
                    while (true) {
                        MemstoreAware latest = memstoreAware.get();
                    ...
                    SpliceLogUtils.warn(LOG, "scan missed do to split after task creation " +
                          ...
                          ((HasRegionServerServices)c.getEnvironment()).getRegionServerServices().getServerName().getHostAndPort()
                            );
