...
public class LazyPartitionServer implements PartitionServer {
  ...
  public LazyPartitionServer(Connection connection, RegionInfo regionInfo, TableName tableName) {
  ...
  public PartitionServerLoad getLoad() {
    try (RegionLocator rl = connection.getRegionLocator(tableName)) {
      ...
      try (Admin admin = connection.getAdmin()) {
        ServerMetrics load = admin.getClusterMetrics().getLiveServerMetrics().get(hrl.getServerName());
