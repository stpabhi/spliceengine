...
public class SIObserver implements RegionCoprocessor, RegionObserver {
  ...
  public void prePut(ObserverContext<RegionCoprocessorEnvironment> e, Put put, WALEdit edit, Durability writeToWAL) throws IOException {
    ...
    try {
      ...
      for (Cell kv : keyValues) {
        @SuppressWarnings("deprecation") byte[] family = kv.getFamilyArray();
        @SuppressWarnings("deprecation") byte[] column = kv.getQualifierArray();
        if (!Bytes.equals(column, SIConstants.PACKED_COLUMN_BYTES)) continue; //skip SI columns

        isSIDataOnly = false;
        @SuppressWarnings("deprecation") byte[] value = kv.getValueArray();
  ...
  private void checkAccess() throws AccessDeniedException {
    ...
    if (user.isPresent()) {
      user.get().getShortName().equalsIgnoreCase("hbase");
    } else return;
